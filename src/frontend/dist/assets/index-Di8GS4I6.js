      ...
      children: tabs.map((tab) => {
        ...
        return /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "button",
          {
            ...
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-xl", children: tab.icon }),
...
const AUTH_KEY = "winverse_user_id";
const ADMIN_KEY = "winverse_admin_token";
function getStoredUserId() {
  const v2 = localStorage.getItem(AUTH_KEY);
  if (!v2) return null;
  try {
    return BigInt(v2);
  } catch {
...
function loadDB() {
  try {
    ...
    if (raw) {
      ...
      if (parsed && typeof parsed === "object" && Array.isArray(parsed.users) && Array.isArray(parsed.phoneToId) && Array.isArray(parsed.phoneToPass) && Array.isArray(parsed.referralToId) && typeof parsed.nextId === "string") {
        ...
        parsed.users = parsed.users.map((u) => ({ ...u, isBanned: u.isBanned ?? false }));
        return parsed;
      }
...
function getMockRound(db) {
  const now2 = Date.now();
  const elapsed = now2 - db.roundStart;
  const remaining = Math.max(0, 3e4 - elapsed);
  const status = remaining > 1e4 ? "betting" : "locked";
  const endTime = BigInt(db.roundStart + 3e4) * BigInt(1e6);
  return {
    id: BigInt(db.roundId),
    startTime: BigInt(db.roundStart) * BigInt(1e6),
    endTime,
    result: [],
    status,
    isManual: false,
...
function pickLowestBetOption(db, roundId) {
  const colours = ["red", "green", "violet"];
  const sizes = ["big", "small"];
  const allOptions = [...colours, ...sizes];
  const totals = {};
  for (const opt of allOptions) {
    totals[opt] = db.bets.filter((b) => b.roundId === roundId && b.betValue === opt && b.status !== "cancelled").reduce((s, b) => s + b.amount, 0);
  }
...
function maybeAdvanceRound(db) {
  const now2 = Date.now();
  let advanced = false;
  const colours = ["red", "green", "violet"];
  const sizes = ["big", "small"];
  while (now2 >= db.roundStart + 3e4) {
    let colour;
    let size;
    ...
    db.roundHistory.push({
      ...
      endTime: String(db.roundStart + 3e4)
    });
    ...
    for (const b of db.bets) {
      if (b.roundId === db.roundId && b.status === "pending") {
        ...
        if (won) {
          ...
          const u = db.users.find((u2) => u2.id === b.userId);
          if (u) {
            u.balance += b.winAmount;
            u.totalWinnings += b.winAmount;
          }
        }
      }
    }
    db.roundId = String(parseInt(db.roundId) + 1);
    db.roundStart = db.roundStart + 3e4;
    advanced = true;
  }
  return advanced;
}
const backendService = {
  ...
  async login(phone, password) {
    ...
    if (user == null ? void 0 : user.isBanned) return [];
  ...
  async getUserById(userId) {
    try {
      const db = loadDB();
      const u = db.users.find((x2) => x2.id === userId.toString());
  ...
  async updateUserName(userId, name) {
    const db = loadDB();
    const idx = db.users.findIndex((x2) => x2.id === userId.toString());
  ...
  async placeBet(userId, roundId, betType, betValue, amount) {
    ...
    db.users = db.users.map((x2) => x2.id === userId.toString() ? { ...x2, balance: x2.balance - amount, totalBets: String(parseInt(x2.totalBets) + 1) } : x2);
    saveDB(db);
    return [BigInt(bid)];
  },
  ...
  async getRoundHistory(limit) {
    const db = loadDB();
    const history = db.roundHistory || [];
    const recent = [...history].reverse().slice(0, Number(limit));
    return recent.map((r) => ({
      id: BigInt(r.id),
      startTime: BigInt(r.startTime) * BigInt(1e6),
      endTime: BigInt(r.endTime) * BigInt(1e6),
      result: [{ colour: r.colour, size: r.size }],
      status: "completed",
      isManual: false,
      manualResult: []
    }));
  ...
  async createDepositRequest(userId, amount, utrNumber, screenshotUrl) {
    const db = loadDB();
  ...
  async createWithdrawRequest(userId, amount, upiId) {
    const db = loadDB();
    const u = db.users.find((x2) => x2.id === userId.toString());
  ...
  async processReferral(newUserId, referralCode) {
    ...
    db.users = db.users.map((u) => u.id === refId ? { ...u, balance: u.balance + 30, referralCount: String(parseInt(u.referralCount) + 1) } : u);
    saveDB(db);
    return true;
  },
  async logout(userId) {
  ...
  async getUserDepositRequests(userId) {
    const db = loadDB();
  ...
  // Admin
  async adminLogin(credential, password) {
  ...
  async getLiveUsers(adminToken) {
    if (adminToken !== "admin123123") return BigInt(0);
    const db = loadDB();
    return BigInt(db.users.filter((u) => u.isActive).length);
  },
  async getPendingDeposits(adminToken) {
    if (adminToken !== "admin123123") return [];
    const db = loadDB();
    return db.deposits.filter((d) => d.status === "pending").map(toDepositPublic);
  },
  async getAllDeposits(adminToken) {
    if (adminToken !== "admin123123") return [];
    const db = loadDB();
    return [...db.deposits].reverse().map(toDepositPublic);
  },
  async getPendingWithdrawals(adminToken) {
    if (adminToken !== "admin123123") return [];
    const db = loadDB();
    return db.withdraws.filter((w) => w.status === "pending").map(toWithdrawPublic);
  },
  async getAllWithdrawals(adminToken) {
    if (adminToken !== "admin123123") return [];
    const db = loadDB();
    return [...db.withdraws].reverse().map(toWithdrawPublic);
  },
  async approveDeposit(requestId, adminToken) {
    if (adminToken !== "admin123123") return false;
    const db = loadDB();
    const d = db.deposits.find((x2) => x2.id === requestId.toString());
    if (!d || d.status !== "pending") return false;
    d.status = "approved";
    d.processedAt = [String(Date.now())];
    const u = db.users.find((x2) => x2.id === d.userId);
    if (u) {
      const bonus = !u.firstDepositDone ? d.amount : 0;
      db.users = db.users.map((x2) => x2.id === d.userId ? { ...x2, balance: x2.balance + d.amount + bonus, hasDeposited: true, firstDepositDone: true, firstDepositBonusGiven: bonus > 0, totalDeposited: x2.totalDeposited + d.amount } : x2);
    }
    saveDB(db);
    return true;
  },
  async rejectDeposit(requestId, adminToken) {
    if (adminToken !== "admin123123") return false;
    const db = loadDB();
    const d = db.deposits.find((x2) => x2.id === requestId.toString());
    if (!d) return false;
  ...
  async approveWithdrawal(requestId, adminToken) {
    if (adminToken !== "admin123123") return false;
    const db = loadDB();
    const w = db.withdraws.find((x2) => x2.id === requestId.toString());
    if (!w || w.status !== "pending") return false;
    w.status = "approved";
    w.processedAt = [String(Date.now())];
    db.users = db.users.map((u) => u.id === w.userId ? { ...u, balance: u.balance - w.amount } : u);
    saveDB(db);
    return true;
  },
  async rejectWithdrawal(requestId, adminToken) {
    if (adminToken !== "admin123123") return false;
    const db = loadDB();
    const w = db.withdraws.find((x2) => x2.id === requestId.toString());
    if (!w) return false;
  ...
  async setNextRoundResult(colour, size, adminToken) {
    if (adminToken !== "admin123123") return false;
    const db = loadDB();
  ...
  async setRandomMode(enabled, adminToken) {
    if (adminToken !== "admin123123") return false;
    const db = loadDB();
  ...
  async getRandomModeStatus(adminToken) {
    if (adminToken !== "admin123123") return true;
    const db = loadDB();
    return db.isRandomMode !== false;
  },
  async setLowestBetWinsMode(enabled, adminToken) {
    if (adminToken !== "admin123123") return false;
    const db = loadDB();
  ...
  async getLowestBetWinsMode(adminToken) {
    if (adminToken !== "admin123123") return false;
    const db = loadDB();
    return db.isLowestBetWinsMode === true;
  },
  async getCurrentRoundBets(adminToken) {
    if (adminToken !== "admin123123") return [];
    const db = loadDB();
    ...
    return options.map((opt) => {
      ...
      return [opt, BigInt(matching.length), matching.reduce((s, b) => s + b.amount, 0)];
    });
  },
  async triggerRoundResult(adminToken) {
    if (adminToken !== "admin123123") return false;
    const db = loadDB();
    const colours = ["red", "green", "violet"];
    const sizes = ["big", "small"];
    ...
    for (const b of db.bets.filter((b2) => b2.roundId === db.roundId && b2.status === "pending")) {
      const won = b.betType === "colour" && b.betValue === colour || b.betType === "size" && b.betValue === size;
      b.status = won ? "won" : "lost";
      if (won) {
        b.winAmount = b.amount * 1.9;
        db.users = db.users.map((u) => u.id === b.userId ? { ...u, balance: u.balance + b.winAmount, totalWinnings: u.totalWinnings + b.winAmount } : u);
      }
    }
    db.roundId = String(parseInt(db.roundId) + 1);
  ...
  async lockCurrentRound(_adminToken) {
    return true;
  },
  async getAdminStats(adminToken) {
    if (adminToken !== "admin123123") return [];
    const db = loadDB();
...
function AccountPage({ userId, onLogout }) {
    ...
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "div",
      {
        ...
        children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { style: { fontFamily: "Plus Jakarta Sans" }, children: "Loading account..." })
        ]
      }
    );
  }
  if (loadingState === "error" || !user) {
    return /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "div",
      {
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { fontSize: "56px" }, children: "⚠️" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "h2",
            {
              className: "text-xl font-black",
              style: { fontFamily: "Plus Jakarta Sans", color: "#ff7162" },
              children: "Account Load Failed"
            }
          ),
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm", style: { color: "#adaaaa", lineHeight: 1.6 }, children: "Could not load your account. Please try again." }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              type: "button",
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "px-4 pt-6 pb-2", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
          "h1",
          {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "px-4 space-y-4", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: cardStyle, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-4", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "div",
              {
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex-1 min-w-0", children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-sm", style: { color: "#adaaaa" }, children: [
                "ID: #",
                user.id.toString()
              ] })
            ] })
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid grid-cols-2 gap-3", children: [
          ...
          ].map(({ label, value, color }) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { ...cardStyle, textAlign: "center" }, children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "p",
              {
                className: "text-xl font-black",
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs mt-1", style: { color: "#adaaaa" }, children: label })
          ] }, label)) }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: cardStyle, children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "font-semibold", children: user.phone })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: cardStyle, children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "p",
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs mt-1", style: { color: "#555" }, children: "Share to earn ₹30 per successful referral" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { ...cardStyle, border: "1px solid rgba(0,207,252,0.15)" }, children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "p",
              {
                className: "font-bold mb-2",
                style: { color: "#00cffc", fontFamily: "Plus Jakarta Sans" },
                children: "🎟️ Help Desk"
              }
            ),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm", style: { color: "#adaaaa" }, children: "Need help? Contact us:" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "a",
              {
                ...
                style: { color: "#9cff93" },
                children: "iamdevloper1309@gmail.com"
              }
            ),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs mt-2", style: { color: "#555" }, children: "Send your query to the email above. We respond within 24 hours." })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
...
function AdminPage() {
  ...
  const roundEndTimeRef = reactExports.useRef(0);
  const [roundBets, setRoundBets] = reactExports.useState(
    []
  );
  const [manualColour, setManualColour] = reactExports.useState("red");
  const [manualSize, setManualSize] = reactExports.useState("big");
  const [randomMode, setRandomMode] = reactExports.useState(true);
  const [lowestBetWinsMode, setLowestBetWinsMode] = reactExports.useState(false);
  ...
  const fetchLiveData = reactExports.useCallback(async () => {
    ...
    if (r.length > 0) {
      const rr = r[0];
      setRound(rr);
      const endMs = Number(rr.endTime) / 1e6;
      roundEndTimeRef.current = endMs;
      const diff = Math.max(0, Math.floor((endMs - Date.now()) / 1e3));
      setTimeLeft(diff);
    }
  ...
  reactExports.useEffect(() => {
    ...
    const ticker = setInterval(() => {
      if (roundEndTimeRef.current > 0) {
        const diff = Math.max(
          0,
          Math.floor((roundEndTimeRef.current - Date.now()) / 1e3)
        );
        setTimeLeft(diff);
      }
    }, 1e3);
  ...
  reactExports.useEffect(() => {
    if (!token) return;
    fetchLiveData();
    const interval = setInterval(fetchLiveData, 3e3);
    return () => clearInterval(interval);
  }, [token, fetchLiveData]);
  const fetchTabData = reactExports.useCallback(async () => {
  ...
  reactExports.useEffect(() => {
    if (!token || activeTab === "live") return;
    fetchTabData();
    const interval = setInterval(fetchTabData, 5e3);
    return () => clearInterval(interval);
  ...
  async function handleToggleRandom() {
    ...
    ue.success(`Random mode ${!randomMode ? "enabled" : "disabled"}`);
  }
  async function handleToggleLowestBetWins() {
    ...
    try {
      ...
      ue.success(`Lowest Bet Wins ${newVal ? "enabled" : "disabled"}`);
    } catch (_) {
      ue.error("Failed to update Lowest Bet Wins mode");
    }
  }
  async function handleBan(userId) {
    if (!token) return;
    await backendService.banUser(userId, token);
    ue.success(`User #${userId} banned`);
    backendService.getAllUsers(token).then(setUsers);
  }
  async function handleUnban(userId) {
    if (!token) return;
    await backendService.unbanUser(userId, token);
    ue.success(`User #${userId} unbanned`);
    backendService.getAllUsers(token).then(setUsers);
  }
  const inputStyle = {
    background: "#1a1919",
        ...
        children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            ...
            children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm mb-6", style: { color: "#adaaaa" }, children: "Admin Dashboard" }),
  ...
  const filteredUsers = users.filter(
    (u) => u.phone.includes(search) || u.name.toLowerCase().includes(search.toLowerCase()) || u.id.toString().includes(search)
  );
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { background: "#0e0e0e", minHeight: "100vh" }, children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      "div",
      {
        ...
        children: tabs2.map((t) => /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            type: "button",
            ...
            style: {
              ...
              borderBottom: activeTab === t.id ? "2px solid #9cff93" : "2px solid transparent"
            },
            children: t.label
          },
          t.id
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "p-4 md:p-6", children: [
      activeTab === "live" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-6 max-w-4xl", children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "div",
            {
              ...
              children: [
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-sm mt-1", style: { color: "#adaaaa" }, children: [
                  "Round #",
                  (round == null ? void 0 : round.id.toString()) || "...",
                  " •",
                  " ",
                  (round == null ? void 0 : round.status) || "..."
                ] })
              ]
            }
          ),
            ...
            "div",
            {
              ...
              children: [
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-sm", style: { color: "#fff" }, children: "Random Mode" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "button",
                    {
                      type: "button",
                      ...
                      style: {
                        background: randomMode ? "rgba(0,255,65,0.15)" : "rgba(255,255,255,0.08)",
                        color: randomMode ? "#9cff93" : "#adaaaa",
                        border: `1px solid ${randomMode ? "rgba(0,255,65,0.3)" : "rgba(255,255,255,0.1)"}`
                      },
                      children: randomMode ? "ON" : "OFF"
                    }
                  )
                ] }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-sm", style: { color: "#fff" }, children: "Lowest Bet Wins" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "button",
                    {
                      type: "button",
                      ...
                      className: "px-4 py-1.5 rounded-full text-sm font-bold",
                      style: {
                        background: lowestBetWinsMode ? "rgba(255,200,0,0.15)" : "rgba(255,255,255,0.08)",
                        color: lowestBetWinsMode ? "#ffc800" : "#adaaaa",
                        border: `1px solid ${lowestBetWinsMode ? "rgba(255,200,0,0.3)" : "rgba(255,255,255,0.1)"}`
                      },
                      children: lowestBetWinsMode ? "ON" : "OFF"
                    }
                  )
                ] }),
                lowestBetWinsMode && /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "p",
                  {
                    className: "text-xs",
                    style: { color: "rgba(255,200,0,0.6)" },
                    children: "Auto: lowest bet option wins next round"
                  }
                ),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-2", children: [
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsxs(
                    "select",
                    {
                      ...
                      children: [
                        /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "big", children: "Big" }),
            ...
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between mb-4", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "p",
                  {
                    className: "text-sm font-semibold",
                    style: { color: "#adaaaa" },
                    children: "BETTING DISTRIBUTION — CURRENT ROUND"
                  }
                ),
                /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "span",
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-3", children: [
                roundBets.map(([option, count, amt]) => {
                  const amtPct = totalAmount > 0 ? amt / totalAmount * 100 : 0;
                  const cntPct = totalCount > 0 ? Number(count) / totalCount * 100 : 0;
                  const colour = betColours[option] || "#888";
                  const isLowest = lowestBetWinsMode && totalAmount > 0 && amt === Math.min(
                    ...roundBets.filter(([, , a]) => a > 0).map(([, , a]) => a)
                  );
                ...
                totalCount === 0 && /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "p",
                  {
                    ...
                    style: { color: "#adaaaa" },
                    children: "No bets placed yet this round"
                  }
                )
              ] })
            ]
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs("table", { className: "w-full", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("thead", { children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "tr",
                  {
                    ...
                    children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "text-left py-2 text-sm", children: "Option" }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "text-center py-2 text-sm", children: "Bets" }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "text-center py-2 text-sm", children: "% of Total" }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "text-right py-2 text-sm", children: "Amount" })
                    ]
                  }
                ) }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("tbody", { children: roundBets.map(([option, count, amt]) => {
                  const pct = totalAmount > 0 ? amt / totalAmount * 100 : 0;
                  const colour = betColours[option] || "#888";
                  const isMax = amt > 0 && amt === Math.max(...roundBets.map(([, , a]) => a));
                  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
                    "tr",
                    {
                      style: {
                        borderBottom: "1px solid rgba(255,255,255,0.03)",
                        background: isMax ? "rgba(255,215,0,0.04)" : "transparent"
                      },
                      children: [
                        /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "py-2 text-sm", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "flex items-center gap-2", children: [
                          /* @__PURE__ */ jsxRuntimeExports.jsx(
                            "span",
                            {
                        ...
                        /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "py-2 text-sm text-center", children: count.toString() }),
                        /* @__PURE__ */ jsxRuntimeExports.jsxs(
                          "td",
                          {
      ...
      activeTab === "users" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-4 max-w-6xl", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "input",
          {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "div",
          {
            ...
            children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { overflowX: "auto" }, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("table", { className: "w-full", style: { minWidth: "700px" }, children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("thead", { style: { background: "#262626" }, children: /* @__PURE__ */ jsxRuntimeExports.jsx("tr", { style: { color: "#adaaaa" }, children: [
                "ID",
                "Phone",
                "Name",
                "Balance",
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs("tbody", { children: [
                filteredUsers.map((u) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "tr",
                  {
                    style: {
                      borderBottom: "1px solid rgba(255,255,255,0.03)",
                      background: u.isBanned ? "rgba(255,113,98,0.04)" : "transparent"
                    },
                    children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsxs(
                        "td",
                      ...
                      /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3 text-sm", children: u.phone }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3 text-sm", children: u.name }),
                      /* @__PURE__ */ jsxRuntimeExports.jsxs(
                        "td",
                        {
                          className: "px-4 py-3 text-sm",
                      ...
                      /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3 text-sm", children: u.totalBets.toString() }),
                      /* @__PURE__ */ jsxRuntimeExports.jsxs("td", { className: "px-4 py-3 text-sm", children: [
                        "₹",
                        u.totalDeposited.toFixed(2)
                      ] }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3", children: u.isBanned ? /* @__PURE__ */ jsxRuntimeExports.jsx(
                        "span",
                        {
                          className: "text-xs px-2 py-0.5 rounded-full font-semibold",
                          style: {
                        ...
                        "span",
                        {
                          ...
                          style: {
                            background: u.isActive ? "rgba(0,255,65,0.15)" : "rgba(255,255,255,0.06)",
                            color: u.isActive ? "#9cff93" : "#adaaaa"
                          },
                          children: u.isActive ? "Online" : "Offline"
                        }
                      ) }),
                      /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-3", children: u.isBanned ? /* @__PURE__ */ jsxRuntimeExports.jsx(
                        "button",
      ...
      activeTab === "deposits" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-5xl", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "p",
          {
        ...
        deposits.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx(
          "div",
          {
            ...
            children: "No deposit requests yet"
          }
        ) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-4", children: deposits.map((d) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            ...
            children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-1.5", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-xs", style: { color: "#adaaaa" }, children: "UTR Number" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "span",
                    {
                      className: "text-sm font-semibold",
                      style: { color: "#fff", wordBreak: "break-all" },
                      children: d.utrNumber || "—"
                    }
                  )
                ] }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-xs", style: { color: "#adaaaa" }, children: "User ID" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "text-sm", style: { color: "#fff" }, children: [
                    "#",
                    d.userId.toString()
                  ] })
                ] }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-xs", style: { color: "#adaaaa" }, children: "Date" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-sm", style: { color: "#adaaaa" }, children: new Date(
                    Number(d.createdAt) / 1e6
                  ).toLocaleString() })
                ] }),
                d.screenshotUrl && d.screenshotUrl !== "N/A" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "span",
                    {
                      className: "text-xs",
              ...
              d.status === "pending" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-3 mt-1", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "button",
                  {
                    type: "button",
      ...
      activeTab === "withdrawals" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-5xl", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "p",
          {
            className: "text-sm font-semibold mb-4",
        ...
        ) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-4", children: withdrawals.map((w) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            className: "rounded-xl p-5 flex flex-col gap-3",
            style: {
            ...
            children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-1.5", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-xs", style: { color: "#adaaaa" }, children: "UPI ID" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "span",
                    {
                      className: "text-sm font-semibold",
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-xs", style: { color: "#adaaaa" }, children: "Date" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-sm", style: { color: "#adaaaa" }, children: new Date(
                    Number(w.createdAt) / 1e6
                  ).toLocaleString() })
                ] })
              ] }),
              w.status === "pending" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex gap-3 mt-1", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "button",
                  {
                    type: "button",
      ...
      activeTab === "stats" && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid grid-cols-2 md:grid-cols-3 gap-4 max-w-3xl", children: [
        {
          label: "Total Users",
        ...
        "div",
        {
          ...
          children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm mt-1", style: { color: "#adaaaa" }, children: label })
          ]
        },
        label
      )) })
...
function AuthPage({
  ...
  const [referralCode, setReferralCode] = reactExports.useState(prefilledReferralCode);
  ...
  async function handleSignup() {
    if (!phone || !password) return ue.error("Fill all fields");
    if (password !== confirmPass) return ue.error("Passwords do not match");
    if (phone.length < 10) return ue.error("Enter valid phone number");
    setLoading(true);
    try {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(
    "div",
    {
      ...
      style: {
        background: "radial-gradient(ellipse at 50% 20%, rgba(0,255,65,0.06) 0%, #0e0e0e 60%)"
      },
      children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "div",
        {
          ...
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-center mb-8", children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { style: { color: "#adaaaa", fontSize: "13px", marginTop: "4px" }, children: "Colour Prediction Game" }),
              prefilledReferralCode && /* @__PURE__ */ jsxRuntimeExports.jsx(
                "div",
                {
                  ...
                  children: "🎁 You were invited! Sign up to claim your bonus."
                }
              )
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "div",
              {
                ...
                children: ["login", "signup"].map((t) => /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "button",
                  {
                    type: "button",
                    onClick: () => setTab(t),
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-4", children: [
              ...
              tab === "signup" && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "input",
                  {
                    style: {
                      ...inputStyle,
                      border: referralCode ? "1px solid rgba(0,255,65,0.4)" : inputStyle.border,
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "button",
                {
                  ...
                  children: loading ? "Please wait..." : tab === "login" ? "Login" : "Create Account"
                }
              ),
              tab === "signup" && /* @__PURE__ */ jsxRuntimeExports.jsx(
                "div",
                {
                  ...
                  children: "🎁 New users get ₹200 signup bonus!"
                }
              )
            ] })
          ]
...
function WinAnimation({ type, amount, onClose }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "button",
    {
      ...
      onClick: onClose,
      children: [
        type === "win" && confettiItems.map((i) => /* @__PURE__ */ jsxRuntimeExports.jsx(
          "div",
          {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-center animate-win-appear px-8", children: type === "win" ? /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
          ...
          amount !== void 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "p",
            {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "mt-4 text-sm", style: { color: "#adaaaa" }, children: "Tap to continue" })
...
function HomePage({ userId }) {
  ...
  const [timeLeft, setTimeLeft] = reactExports.useState(30);
  const [selectedColour, setSelectedColour] = reactExports.useState(
    null
  );
  const [selectedSize, setSelectedSize] = reactExports.useState(null);
  const [betAmount, setBetAmount] = reactExports.useState(20);
  const [customAmount, setCustomAmount] = reactExports.useState("");
  const [roundBets, setRoundBets] = reactExports.useState([]);
  const [animation, setAnimation] = reactExports.useState(null);
  const [loading, setLoading] = reactExports.useState(false);
  const roundBetsRef = reactExports.useRef([]);
  const lastRoundIdRef = reactExports.useRef(null);
  const userIdRef = reactExports.useRef(userId);
  userIdRef.current = userId;
  function updateRoundBets(bets) {
    roundBetsRef.current = bets;
    setRoundBets(bets);
  }
  const fetchUserRef = reactExports.useRef(async () => {
    const res = await backendService.getUserById(userIdRef.current);
    if (res.length > 0) setBalance(res[0].balance);
  });
  const fetchHistoryRef = reactExports.useRef(async () => {
    const res = await backendService.getRoundHistory(BigInt(10));
    setHistory(res);
  });
  const fetchRoundRef = reactExports.useRef(async () => {
    const res = await backendService.getCurrentRound();
    if (res.length > 0) {
      const r = res[0];
      setRound(r);
      const endMs = Number(r.endTime) / 1e6;
      const diff = Math.max(0, Math.floor((endMs - Date.now()) / 1e3));
      setTimeLeft(diff);
  ...
  const presets = [20, 50, 100, 500, 1e3];
  const timerPercent = timeLeft / 30 * 100;
  async function handlePlaceBet() {
    const amount = customAmount ? Number.parseFloat(customAmount) : betAmount;
    if (!amount || Number.isNaN(amount))
      return ue.error("Enter valid amount");
    if (!selectedColour && !selectedSize)
      return ue.error("Select colour or size");
    if (isLocked) return ue.error("Bets are locked!");
    if (betCount >= 3) return ue.error("Max 3 bets per round");
    if (amount > balance) return ue.error("Insufficient balance");
    if (!round) return ue.error("No active round");
    setLoading(true);
    try {
      if (selectedColour) {
        ...
        ue.success(`Bet placed: ₹${amount} on ${selectedColour}`);
      }
      if (selectedSize) {
        ...
        setBalance((prev) => prev - amount);
        ue.success(`Bet placed: ₹${amount} on ${selectedSize}`);
      }
    } finally {
      setLoading(false);
    }
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      children: [
        animation && /* @__PURE__ */ jsxRuntimeExports.jsx(
          WinAnimation,
          {
            type: animation.type,
          ...
          "div",
          {
            ...
            children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3", children: [
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { color: "#adaaaa", fontSize: "20px" }, children: "🔔" })
              ] })
            ]
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "px-4 pt-3 space-y-4", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "div",
            {
              className: "rounded-2xl p-4 flex items-center justify-between",
              style: {
                background: "linear-gradient(135deg, rgba(0,255,65,0.15) 0%, rgba(0,207,252,0.1) 100%)",
                border: "1px solid rgba(0,255,65,0.2)"
              },
              children: [
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "p",
                    {
                      ...
                      children: "🎁 1st Deposit Pe 100% Bonus!"
                    }
                  ),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs mt-1", style: { color: "#adaaaa" }, children: "Deposit karo, double pao" })
                ] }),
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "button",
                  {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "div",
            {
              ...
              children: [
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between mb-4", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs", style: { color: "#adaaaa" }, children: "Period" }),
                    /* @__PURE__ */ jsxRuntimeExports.jsxs(
                      "p",
                      {
                    ...
                    /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "div",
                      {
                        ...
                        style: {
                          ...
                          textShadow: isLocked ? "0 0 15px rgba(255,113,98,0.5)" : "0 0 15px rgba(0,255,65,0.4)"
                        },
                    ...
                    children: /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "div",
                      {
                        ...
                        style: {
                          ...
                          boxShadow: isLocked ? "0 0 8px rgba(255,113,98,0.5)" : "0 0 8px rgba(0,255,65,0.5)"
                        }
                      }
                    )
                  }
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex gap-3 justify-center mb-5", children: [
                  {
                    value: "green",
                    label: "Green",
                    bg: "#9cff93",
                ...
                ].map(({ value, label, bg, shadow }) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "button",
                  {
                    type: "button",
                    onClick: () => !isLocked && setSelectedColour(selectedColour === value ? null : value),
                    disabled: isLocked,
                      ...
                      /* @__PURE__ */ jsxRuntimeExports.jsx(
                        "div",
                        {
                          ...
                          style: {
                            ...
                            border: `3px solid ${bg}`,
                            boxShadow: selectedColour === value ? `0 0 20px ${shadow}, 0 0 40px ${shadow}` : "none",
                            transform: selectedColour === value ? "scale(1.1)" : "scale(1)"
                          },
                          children: selectedColour === value && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { fontSize: "24px" }, children: "✓" })
                        }
                      ),
                      /* @__PURE__ */ jsxRuntimeExports.jsx(
                        "span",
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex gap-3 mb-5", children: ["big", "small"].map((s) => /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "button",
                  {
                    type: "button",
                    onClick: () => !isLocked && setSelectedSize(selectedSize === s ? null : s),
                    disabled: isLocked,
                    className: "flex-1 py-3 rounded-xl font-bold capitalize transition-all",
                    style: {
                      background: selectedSize === s ? "rgba(0,255,65,0.15)" : "#262626",
                      border: `2px solid ${selectedSize === s ? "#9cff93" : "rgba(255,255,255,0.08)"}`,
                      color: selectedSize === s ? "#9cff93" : "#adaaaa",
                      boxShadow: selectedSize === s ? "0 0 12px rgba(0,255,65,0.3)" : "none",
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex gap-2 flex-wrap mb-3", children: presets.map((amt) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "button",
                  {
                    type: "button",
                    onClick: () => {
                    ...
                    style: {
                      background: betAmount === amt && !customAmount ? "rgba(0,255,65,0.15)" : "#262626",
                      border: `1px solid ${betAmount === amt && !customAmount ? "#9cff93" : "rgba(255,255,255,0.08)"}`,
                      color: betAmount === amt && !customAmount ? "#9cff93" : "#adaaaa"
                    },
                    children: [
                      "₹",
                      amt
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between mb-3", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "text-xs", style: { color: "#adaaaa" }, children: [
                    betCount,
                    "/3 bets this round"
                  ] }),
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "text-xs", style: { color: "#adaaaa" }, children: [
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "button",
                  {
                    ...
                    style: {
                      ...
                      fontFamily: "Plus Jakarta Sans",
                      boxShadow: isLocked || betCount >= 3 ? "none" : "0 0 20px rgba(0,255,65,0.3)",
                      fontSize: "16px"
                    },
                    children: loading ? "Placing..." : isLocked ? "🔒 Bets Locked" : betCount >= 3 ? "Max Bets Reached" : "🎯 Place Bet"
                  }
                )
              ]
            }
          ),
          roundBets.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "div",
            {
              ...
              children: [
                ...
                roundBets.map((b) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                  "div",
                  {
                    className: "flex justify-between text-sm py-1",
                    children: [
                      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { color: "#fff" }, children: b.betValue.toUpperCase() }),
                      /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { style: { color: "#9cff93" }, children: [
                        "₹",
                        b.amount.toFixed(2)
                      ] })
                    ]
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsxs("table", { className: "w-full text-sm", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("thead", { children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
                    "tr",
                    {
                      ...
                      children: [
                        /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "text-left py-2", children: "Period" }),
                        /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "text-center py-2", children: "Colour" }),
                        /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "text-center py-2", children: "Size" })
                      ]
                    }
                  ) }),
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("tbody", { children: [
                    history.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("tr", { children: /* @__PURE__ */ jsxRuntimeExports.jsx(
                      "td",
                      {
                        colSpan: 3,
                    ...
                    history.map((r) => {
                      ...
                      return /* @__PURE__ */ jsxRuntimeExports.jsxs(
                        "tr",
                        {
                          ...
                          children: [
                            /* @__PURE__ */ jsxRuntimeExports.jsxs("td", { className: "py-2", style: { color: "#adaaaa" }, children: [
                              "#",
                              r.id.toString()
                            ] }),
                            /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "py-2 text-center", children: result && /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "flex items-center justify-center gap-1.5", children: [
                              /* @__PURE__ */ jsxRuntimeExports.jsx(
                                "span",
                                {
                              ...
                              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { color: "#fff", fontSize: "12px" }, children: result.colour })
                            ] }) }),
                            /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "py-2 text-center", children: result && /* @__PURE__ */ jsxRuntimeExports.jsx(
                              "span",
                              {
                                className: "px-2 py-0.5 rounded text-xs font-medium",
                                style: {
                                  background: result.size === "big" ? "rgba(0,207,252,0.15)" : "rgba(255,113,98,0.15)",
                                  color: result.size === "big" ? "#00cffc" : "#ff7162"
                                },
...
function ReferralPage({ userId }) {
  const [code, setCode] = reactExports.useState("");
  const [count, setCount] = reactExports.useState(0);
  const [earnings, setEarnings] = reactExports.useState(0);
  reactExports.useEffect(() => {
    ...
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "px-4 pt-6 pb-4", children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm mt-1", style: { color: "#adaaaa" }, children: "Earn ₹30 bonus for each referral (3 free bets × ₹10 each)" })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "px-4 space-y-4", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: cardStyle, children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-2 gap-3", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { ...cardStyle, textAlign: "center" }, children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "p",
                {
                  className: "text-3xl font-black",
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm mt-1", style: { color: "#adaaaa" }, children: "Total Referrals" })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { ...cardStyle, textAlign: "center" }, children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "p",
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm mt-1", style: { color: "#adaaaa" }, children: "Total Earned" })
            ] })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "div",
            {
              className: "rounded-2xl p-5",
              style: {
                background: "linear-gradient(135deg, rgba(0,255,65,0.08) 0%, rgba(0,207,252,0.05) 100%)",
              ...
              children: [
                ...
                ].map(({ icon, text }) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-start gap-3", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { fontSize: "16px" }, children: icon }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "p",
                    {
                      className: "text-sm",
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "div",
            {
              ...
              children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs font-bold mb-2", style: { color: "#00cffc" }, children: "FREE BET RULES" }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-xs", style: { color: "#adaaaa", lineHeight: 1.6 }, children: [
...
function WalletPage({ userId }) {
  ...
  async function handleDeposit() {
    ...
    if (!amount || amount < 100) return ue.error("Minimum deposit ₹100");
    if (!utrNumber) return ue.error("Enter UTR number");
    setLoading(true);
    try {
      ...
      if (res.length === 0) return ue.error("Request failed");
      ue.success("Deposit request submitted! Awaiting admin approval.");
      setDepositAmount("");
      setUtrNumber("");
      setScreenshotUrl("");
  ...
  async function handleWithdraw() {
    ...
    if (!amount || amount < 100) return ue.error("Minimum withdrawal ₹100");
    if (!upiId) return ue.error("Enter UPI ID");
    setLoading(true);
    try {
      ...
      if (res.length === 0)
        return ue.error("Cannot withdraw. Ensure you have deposited ₹200+");
  ...
  const canWithdraw = (user == null ? void 0 : user.hasDeposited) && user.totalDeposited >= 200;
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      style: {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            ...
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm", style: { color: "#adaaaa" }, children: "Your Balance" }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "p",
                {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "px-4 mb-4", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex rounded-xl p-1", style: { background: "#1a1919" }, children: ["deposit", "withdraw"].map((t) => /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            type: "button",
            onClick: () => setTab(t),
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "px-4 space-y-4", children: [
          tab === "deposit" && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "div",
              {
                    ...
                    "div",
                    {
                      ...
                      children: /* @__PURE__ */ jsxRuntimeExports.jsx(
                        "span",
                        {
                          className: "text-sm font-semibold",
                          style: { color: "#9cff93" },
                          children: "🎁 First deposit gets 100% bonus!"
                        }
                      )
                    }
                  ),
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                    ...
                    /* @__PURE__ */ jsxRuntimeExports.jsxs(
                      "div",
                      {
                        ...
                        children: [
                          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { color: "#9cff93", fontSize: "15px" }, children: "winverse@naviaxis" }),
                          /* @__PURE__ */ jsxRuntimeExports.jsx(
                            "button",
                            {
                              type: "button",
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "div",
                    {
                      ...
                      children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "text-center", children: [
                        ...
                        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm", style: { color: "#adaaaa" }, children: "QR Code" }),
                        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-xs", style: { color: "#555" }, children: "winverse@naviaxis" })
                      ] })
                    }
                  ),
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
            ...
            deposits.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "div",
              {
                ...
                children: [
                  ...
                  deposits.map((d) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                    "div",
                    {
                      ...
                      children: [
                        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                          ...
                          /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-xs", style: { color: "#adaaaa" }, children: [
                            "UTR: ",
                            d.utrNumber
                          ] })
                        ] }),
          ...
          tab === "withdraw" && /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
            !canWithdraw ? /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "div",
              {
                ...
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { fontSize: "48px", marginBottom: "12px" }, children: "🔒" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "p",
                    {
                      className: "font-bold text-lg mb-2",
                      style: { fontFamily: "Plus Jakarta Sans", color: "#ff7162" },
                      children: "Withdrawal Locked"
                    }
                  ),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-sm", style: { color: "#adaaaa" }, children: "Deposit a total of ₹200 or more to unlock withdrawals." }),
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-sm mt-2", style: { color: "#adaaaa" }, children: [
                    "Current deposits:",
                    " ",
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { style: { color: "#9cff93" }, children: [
                      "₹",
                      (user == null ? void 0 : user.totalDeposited.toFixed(2)) || "0.00"
                    ] })
                  ] })
                ]
              }
                ...
                children: [
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "button",
                    {
                      ...
                      children: loading ? "Submitting..." : "Submit Withdrawal Request"
                    }
                  )
                ]
              }
            ...
            withdrawals.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "div",
              {
                ...
                children: [
                  ...
                  withdrawals.map((w) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                    "div",
                    {
                      className: "flex justify-between items-center py-2",
                      style: { borderBottom: "1px solid rgba(255,255,255,0.04)" },
...
function App() {
  ...
  if (isAdmin) {
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-h-screen", style: { background: "#0e0e0e" }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(AdminPage, {}),
      /* @__PURE__ */ jsxRuntimeExports.jsx(Toaster, {})
    ] });
  }
  if (!userId) {
    return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-h-screen", style: { background: "#0e0e0e" }, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(AuthPage, { onLogin: handleLogin, prefilledReferralCode: refCode || "" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(Toaster, {})
    ] });
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-h-screen pb-20", style: { background: "#0e0e0e" }, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-[430px] mx-auto", children: [
      activeTab === "home" && /* @__PURE__ */ jsxRuntimeExports.jsx(HomePage, { userId, onBalanceUpdate: () => {
      } }),
      activeTab === "referral" && /* @__PURE__ */ jsxRuntimeExports.jsx(ReferralPage, { userId }),
      activeTab === "wallet" && /* @__PURE__ */ jsxRuntimeExports.jsx(WalletPage, { userId }),
      activeTab === "account" && /* @__PURE__ */ jsxRuntimeExports.jsx(AccountPage, { userId, onLogout: () => setUserId(null) })
    ] }),
